...
pub fn write(cli: Cli) -> Result<(), CliError> {
    ...
    match cli.command {
        ...
        Command::Interactive(args) => interactive_command(args, cli.dry_run).map_err(Into::into),
...
fn interactive_command(args: InteractiveArgs, dry_run: bool) -> Result<(), String> {
    let mut app = InteractiveApp::new(&args.path, dry_run)?;
    let mut terminal = InteractiveTerminal::enter()?;

    loop {
        app.drain_preview_results();
        app.drain_write_results();
...
struct InteractiveApp {
    ...
    write_sender: Sender<InteractiveWriteResult>,
    write_receiver: Receiver<InteractiveWriteResult>,
    write_request_id: u64,
    write_loading: bool,
    write_loading_visible: bool,
    write_loading_deadline: Option<Instant>,
    mode: InteractiveMode,
    write_editor: InteractiveWriteEditor,
    input_cursor_visible: bool,
    input_cursor_deadline: Instant,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum InteractiveTagKind {
    Standard,
    Custom,
}

#[derive(Debug, Clone, PartialEq)]
struct InteractiveWriteEditor {
    image: Option<PathBuf>,
    rows: Vec<InteractiveTagRow>,
    selected: usize,
    status: Option<InteractiveStatus>,
    state: InteractiveWriteState,
}

impl Default for InteractiveWriteEditor {
    fn default() -> Self {
        Self {
            image: None,
            rows: Vec::new(),
            selected: 0,
            status: None,
            state: InteractiveWriteState::Browsing,
        }
    }
}
...
struct InteractiveStatus {
    kind: InteractiveStatusKind,
    message: String,
}

#[derive(Debug, Clone, PartialEq)]
enum InteractiveWriteState {
    Browsing,
    Editing {
        name: String,
        kind: InteractiveTagKind,
...
#[derive(Debug, Clone, PartialEq)]
struct InteractiveWriteResult {
    request_id: u64,
    kind: InteractiveEntryKind,
    path: PathBuf,
    rows: Result<Vec<InteractiveTagRow>, String>,
}
...
impl InteractiveApp {
    fn new(path: &Path, dry_run: bool) -> Result<Self, String> {
        ...
        let mut app = Self {
            ...
            write_request_id: 0,
            write_loading: false,
            write_loading_visible: false,
            write_loading_deadline: None,
            mode: InteractiveMode::Read,
            write_editor: InteractiveWriteEditor::default(),
            input_cursor_visible: true,
            input_cursor_deadline: Instant::now() + INTERACTIVE_CURSOR_TICK,
        };
    ...
    fn handle_event(&mut self, event: Event) -> Result<bool, String> {
        ...
        match key {
            ...
            KeyEvent {
                ...
                if self.cancel_write_input() {
                    Ok(false)
                } else if matches!(
                    self.focus,
                    InteractiveFocus::Preview | InteractiveFocus::WriteEditor
                ) {
    ...
    fn handle_focused_key(&mut self, key: KeyEvent) -> Result<bool, String> {
        match self.focus {
            ...
            InteractiveFocus::WriteEditor => self.handle_write_key(key).map(|()| false),
        }
    }
    ...
    fn handle_write_key(&mut self, key: KeyEvent) -> Result<(), String> {
        match &mut self.write_editor.state {
            InteractiveWriteState::Browsing => match key {
                KeyEvent {
                    code: KeyCode::Up, ..
                } => self.select_previous_write_row(),
                KeyEvent {
                    code: KeyCode::Down,
                    ..
                } => self.select_next_write_row(),
                KeyEvent {
                    code: KeyCode::Enter,
                    ..
    ...
    fn write_input_active(&self) -> bool {
    ...
    fn toggle_mode(&mut self) {
        ...
        if self.mode == InteractiveMode::Write {
            ...
            self.write_loading_deadline = None;
        }
    }
    ...
    fn selected_file(&self) -> Option<PathBuf> {
        self.entries
            .get(self.selected)
            .filter(|entry| entry.kind == InteractiveEntryKind::File)
            .map(|entry| entry.path.clone())
    }

    fn request_write_editor_for_selection(&mut self) {
        self.write_request_id = self.write_request_id.wrapping_add(1);
        let Some(image) = self.selected_file() else {
            self.write_editor.image = None;
            self.write_editor.rows.clear();
            self.write_editor.selected = 0;
            self.write_editor.status = None;
            self.write_editor.state = InteractiveWriteState::Browsing;
            self.write_loading = false;
            self.write_loading_visible = false;
        ...
        self.write_editor.image = Some(image.clone());
        self.write_editor.state = InteractiveWriteState::Browsing;
        self.write_editor.status = None;
        ...
        thread::spawn(move || {
            let rows = interactive_tag_rows(&image);
            let _ = sender.send(InteractiveWriteResult {
                request_id,
    ...
    fn start_edit_selected_write_row(&mut self) {
    ...
    fn apply_interactive_tag(
        ...
        if kind == InteractiveTagKind::Standard && writable_exif_tag(&name, &value).is_none() {
            ...
            self.write_editor.state = InteractiveWriteState::Editing { name, kind, input };
            return Ok(());
        }
    ...
    fn drain_write_results(&mut self) {
        while let Ok(result) = self.write_receiver.try_recv() {
    ...
    fn apply_write_result(&mut self, result: InteractiveWriteResult) {
        if result.request_id != self.write_request_id
            || !self.selected_entry_matches(result.kind, &result.path)
        {
            return;
        }
        ...
        self.write_loading_visible = false;
        self.write_loading_deadline = None;
        self.write_editor.image = Some(result.path);
        match result.rows {
            Ok(rows) => {
    ...
    fn tick_write_loading(&mut self) {
        if !self.write_loading {
            return;
        }

        if !self.write_loading_visible
            && self
                .write_loading_deadline
                .is_some_and(|deadline| Instant::now() >= deadline)
        {
            self.write_loading_visible = true;
        }
    }

    fn tick_input_cursor(&mut self) {
        if !self.write_input_active() {
            self.input_cursor_visible = true;
            self.input_cursor_deadline = Instant::now() + INTERACTIVE_CURSOR_TICK;
            return;
        }

        if Instant::now() >= self.input_cursor_deadline {
            self.input_cursor_visible = !self.input_cursor_visible;
            self.input_cursor_deadline = Instant::now() + INTERACTIVE_CURSOR_TICK;
        }
    }
...
fn yaml_value_from_input(input: &str) -> YamlValue {
    serde_yaml::from_str::<YamlValue>(input)
        .ok()
        .filter(|value| !matches!(value, YamlValue::Null))
        .unwrap_or_else(|| YamlValue::String(input.trim().to_string()))
}

fn yaml_value_to_input(value: &YamlValue) -> Option<String> {
    match value {
...
fn filtered_writable_standard_tags(query: &str) -> Vec<&'static str> {
    let query = query.trim().to_ascii_lowercase();
    WRITABLE_STANDARD_EXIF_TAG_NAMES
        .iter()
        .copied()
...
fn apply_interactive_tag_to_image(
    image: &Path,
    name: &str,
    value: YamlValue,
    kind: InteractiveTagKind,
    ...
    let tags = match kind {
        InteractiveTagKind::Standard => vec![WriteTag {
            name: name.to_string(),
            value,
        }],
        InteractiveTagKind::Custom => {
            let mut custom_tags = read_metadata(image)
                .map(|metadata| custom_tags_from_exif(&metadata.exif))
                .unwrap_or_default();
            if let Some(existing) = custom_tags.iter_mut().find(|tag| tag.name == name) {
    ...
    Ok(apply_tags_to_image(
        image,
        &tags,
        dry_run,
        &interactive_write_args(),
        None,
    ))
}
...
fn selected_preview_title(app: &InteractiveApp) -> &'static str {
    if app.mode == InteractiveMode::Write && app.selected_file().is_some() {
        return "Write";
    }
...
fn interactive_mode_label(mode: InteractiveMode) -> &'static str {
    match mode {
        InteractiveMode::Read => "READ",
        InteractiveMode::Write => "WRITE",
    }
}
...
fn render_interactive(frame: &mut ratatui::Frame<'_>, app: &mut InteractiveApp) {
    ...
    let files = List::new(items)
        .block(
            Block::default()
                .title(format!(
                    " {} [{}] ",
                    display_path(&app.current_dir),
                    interactive_mode_label(app.mode)
                ))
                .borders(Borders::ALL),
        )
        .highlight_style(
    ...
    let preview_text = if app.mode == InteractiveMode::Write && app.selected_file().is_some() {
        Text::from(write_editor_lines(app))
    } else {
        match &app.preview {
...
fn write_editor_lines(app: &InteractiveApp) -> Vec<Line<'static>> {
    ...
    if app.dry_run {
        lines.push(Line::from(Span::styled(
            "dry-run: files will not be changed",
...
fn append_write_browsing_lines(
    ...
    if editor.rows.is_empty() {
        ...
        lines.push(Line::from(
            "Press a to add a writable standard or custom tag.",
        ));
        return;
    }
...
fn append_check_file_group(output: &mut String, first_group: &mut bool, report: &CheckOutput) {
    ...
    if let Some(path) = &report.metadata_path {
        append_check_file_row(
            output,
            "metadata file",
            format_success_status(&format!("found {}", path.display())),
        );
    } else {
        append_check_file_row(output, "metadata file", "skipped");
    }

    if report.yaml_ok {
        append_check_file_row(output, "YAML format", format_success_status("ok"));
    } else {
        append_check_file_row(output, "YAML format", "skipped");
    }
...
fn append_check_frames_group(output: &mut String, first_group: &mut bool, report: &CheckOutput) {
    ...
    if frames.frame_number_count > 0 {
        append_check_frame_summary_row(output, "frames", frames.frame_number_count);
        append_check_frame_summary_row(output, "files", frames.file_count);
    ...
    for frame in &frames.frames {
        output.push_str(&format!("{}\n", format_check_frame_title(frame)));
...
fn append_check_overview_group(output: &mut String, first_group: &mut bool, report: &CheckOutput) {
    ...
    append_check_row(output, "errors", format_check_error_count(errors));
    append_check_row(output, "warnings", format_check_warning_count(warnings));

    if errors > 0 {
        append_check_row(output, "validation", format_error_status("error"));
    } else if warnings > 0 {
        append_check_row(output, "validation", format_warning_status("warning"));
    } else {
        append_check_row(output, "validation", format_success_status("success"));
    }
}
...
fn append_check_row(output: &mut String, label: &str, value: impl std::fmt::Display) {
    append_check_row_with_width(output, label, value, CHECK_ROW_LABEL_WIDTH);
}
...
fn append_check_frame_summary_row(output: &mut String, label: &str, value: impl std::fmt::Display) {
    append_check_row_with_width(output, label, value, CHECK_FRAME_SUMMARY_LABEL_WIDTH);
}

fn append_check_row_with_width(
    output: &mut String,
    label: &str,
...
fn format_check_frame_title(frame: &FrameReport) -> String {
    ...
    if !frame.errors.is_empty() || !frame.invalid_tags.is_empty() {
        ...
        format!("{} {title}", check_success_icon())
    }
}

fn format_success_status(text: &str) -> String {
    format!("{} {text}", check_success_icon())
}
...
fn check_warning_icon() -> String {
    "⚠".yellow().to_string()
}

fn check_error_icon() -> String {
    "✗".red().to_string()
}
...
fn check_tag_keys(mapping: &Mapping, context: &str) -> Result<(), String> {
...
const WRITABLE_STANDARD_EXIF_TAG_NAMES: &[&str] = &[
    "Artist",
    "Copyright",
    "CreateDate",
    "DateTimeOriginal",
...
mod tests {
    ...
    #[test]
    fn interactive_tab_toggles_mode_when_not_editing() {
        let directory = temporary_test_directory("interactive-tab-toggle");
        std::fs::write(directory.join("image.jpg"), [0xff, 0xd8, 0xff, 0xd9])
            .expect("jpg should be written");
    ...
    #[test]
    fn interactive_tab_does_not_toggle_mode_while_editing() {
        let directory = temporary_test_directory("interactive-tab-editing");
        std::fs::write(directory.join("image.jpg"), [0xff, 0xd8, 0xff, 0xd9])
            .expect("jpg should be written");
    ...
    #[test]
    fn interactive_read_mode_right_on_file_still_focuses_preview() {
        let directory = temporary_test_directory("interactive-read-right");
        std::fs::write(directory.join("image.jpg"), [0xff, 0xd8, 0xff, 0xd9])
            .expect("jpg should be written");
    ...
    #[test]
    fn interactive_write_mode_right_on_file_focuses_editor() {
        let directory = temporary_test_directory("interactive-write-right");
        ...
        assert!(
            app.write_editor
                .rows
                .iter()
                .any(|row| row.name == "Make" && row.kind == InteractiveTagKind::Standard)
        );
    ...
    #[test]
    fn interactive_matching_write_result_updates_editor_rows() {
        let directory = temporary_test_directory("interactive-write-result");
        std::fs::write(directory.join("image.jpg"), [0xff, 0xd8, 0xff, 0xd9])
            .expect("jpg should be written");
        let mut app = InteractiveApp::new(&directory, false).expect("app should initialize");
        app.selected = app
        ...
        assert_eq!(app.write_editor.rows[0].name, "Model");

        let _ = std::fs::remove_dir_all(directory);
    }

    #[test]
    fn interactive_stale_write_result_is_discarded() {
        let directory = temporary_test_directory("interactive-stale-write");
        ...
        app.request_write_editor_for_selection();
        let stale = write_result_for_selected(&app, Ok(Vec::new()));
        app.selected = app
            .entries
            .iter()
        ...
        app.apply_write_result(stale);

        assert!(app.write_loading);
        assert_eq!(
            app.write_editor.image,
            Some(fs::canonicalize(directory.join("b.jpg")).unwrap())
        );
    ...
    #[test]
    fn interactive_write_request_clears_for_non_file_selection() {
        let directory = temporary_test_directory("interactive-write-non-file");
        let nested = directory.join("nested");
        std::fs::create_dir(&nested).expect("nested should be created");
        ...
        assert!(!app.write_loading);
        assert!(app.write_editor.image.is_none());
        assert!(app.write_editor.rows.is_empty());

        let _ = std::fs::remove_dir_all(directory);
    }

    #[test]
    fn interactive_writable_standard_picker_excludes_unsupported_standard_tags() {
        assert!(WRITABLE_STANDARD_EXIF_TAG_NAMES.contains(&"Make"));
        assert!(!WRITABLE_STANDARD_EXIF_TAG_NAMES.contains(&"FilmRoll"));
        assert!(filtered_writable_standard_tags("make").contains(&"Make"));
    }

    #[test]
    fn interactive_standard_write_updates_image() {
        let directory = temporary_test_directory("interactive-standard-write");
        let image = directory.join("image.jpg");
        std::fs::write(&image, [0xff, 0xd8, 0xff, 0xd9]).expect("jpg should be written");

        let result = apply_interactive_tag_to_image(
            &image,
            "Make",
            YamlValue::String("Nikon".to_string()),
            InteractiveTagKind::Standard,
            false,
        )
        .expect("interactive write should run");

        assert_eq!(result.written, 1);
        assert!(exif_tag_names(&image).contains("Make"));

        let _ = std::fs::remove_dir_all(directory);
    }

    #[test]
    fn interactive_custom_write_preserves_existing_custom_tags() {
        let directory = temporary_test_directory("interactive-custom-write");
        let image = directory.join("image.jpg");
        write_test_exif_tags(&image, [("FilmRoll", "35")]);

        let result = apply_interactive_tag_to_image(
            &image,
            "FilmName",
            YamlValue::String("Double-X".to_string()),
            InteractiveTagKind::Custom,
            false,
        )
        .expect("interactive custom write should run");

        assert_eq!(result.written, 2);
        let metadata = read_metadata(&image).expect("metadata should read");
        let custom_tags = custom_tags_from_exif(&metadata.exif);
        assert!(custom_tags.iter().any(|tag| tag.name == "FilmRoll"));
        assert!(custom_tags.iter().any(|tag| tag.name == "FilmName"));

        let _ = std::fs::remove_dir_all(directory);
    }

    #[test]
    fn interactive_dry_run_write_does_not_change_image() {
        let directory = temporary_test_directory("interactive-dry-run-write");
        let image = directory.join("image.jpg");
        std::fs::write(&image, [0xff, 0xd8, 0xff, 0xd9]).expect("jpg should be written");
        ...
        .expect("interactive dry-run should run");
    ...
    #[test]
    fn interactive_successful_edit_requests_async_refresh() {
        let directory = temporary_test_directory("interactive-edit-refresh");
        let image = directory.join("image.jpg");
        std::fs::write(&image, [0xff, 0xd8, 0xff, 0xd9]).expect("jpg should be written");

        let mut app = InteractiveApp::new(&directory, true).expect("app should initialize");
        app.selected = app
            .entries
            .iter()
            .position(|entry| entry.label == "image.jpg")
            .expect("image should exist");
        app.mode = InteractiveMode::Write;
        app.write_editor.image = Some(image);
        let request_id = app.write_request_id;

        app.apply_interactive_tag(
            "Make".to_string(),
            InteractiveTagKind::Standard,
            "Nikon".to_string(),
        )
    ...
    fn interactive_input_text_renders_without_fake_cursor() {
        ...
        assert!(!app.input_cursor_visible);
        let rendered = write_editor_lines(&app)
            .iter()
            .map(line_text)
            .collect::<Vec<_>>()
    ...
    fn write_result_for_selected(
        ...
        InteractiveWriteResult {
            request_id: app.write_request_id,
            kind: entry.kind,
            path: entry.path.clone(),
            rows,
        }
...
"#,
    ...
    #[test]
    fn check_metadata_accepts_non_standard_exif_tags_without_warnings() {
        let yaml = serde_yaml::from_str::<YamlValue>(
            r#"
exif:
  Make: Nikon
...
"#,
    ...
    #[test]
    fn check_metadata_accepts_non_standard_frame_tags_and_location_special_key() {
...
"#,
    ...
    #[test]
    fn formats_missing_location_warning_in_yellow() {
        colored::control::set_override(true);
...
"#,
        ...
        assert!(!rendered.contains("NotARealExifTag"));
        assert!(!rendered.contains("AlsoNotARealExifTag"));
    ...
    fn check_overview_renders_success_with_warnings() {
        ...
        assert!(rendered.contains("validation    ⚠ warning"));
    }
...
"#,
    ...
    #[test]
    fn check_overview_colours_warning_status() {
        colored::control::set_override(true);
...
"#,
        ...
        let output = build_check_output(Some(&metadata));
        let rendered = strip_ansi_codes(&format_check_output(&output));
        let frame_two = rendered
            .find("✓ one.jpg ← 2")
            .expect("frame 2 should render");
...
"#,
        ...
        assert!(rendered.contains("✓ 01.jpg ← 1"));
        assert!(!rendered.contains("standard tags"));
        assert!(!rendered.contains("unknown tags"));
